window.onload = () => {
  ...
  fetch("https://bizbot-khpq.onrender.com/api/chatbots/count", {
    ...
    .then(
      (data) =>
        (document.getElementById("chatbot-count").textContent = data.count)
    )
    .catch((error) => {
  ...
  fetch("https://bizbot-khpq.onrender.com/api/faqs/count", {
    ...
    .then((response) => response.json())
    .then(
      (data) => (document.querySelector(".faq-number").textContent = data.count)
    )
    .catch((error) => {
    ...
    feedbacks.forEach((feedback) => {
      ...
      feedbackElement.innerHTML = `
              ...
              <div class="feedback-content">
                  <div><span class="prompt">&gt;</span> <strong>Chatbot:</strong> <span>${
                    chatbotSelect.options[chatbotSelect.selectedIndex].text
                  }</span></div>
                  <div><span class="prompt">&gt;</span> <strong>Rating:</strong> ${
                    feedback.rating
                  }</div>
                  <div><span class="prompt">&gt;</span> <strong>Feedback:</strong> ${feedback.feedback.substring(
                    0,
                    100
                  )}${feedback.feedback.length > 100 ? "..." : ""}</div>
              </div>
          `;
          ...
          html: `
                      <div style="text-align: left;">
                          <p><strong>Chatbot:</strong> ${
                            chatbotSelect.options[chatbotSelect.selectedIndex]
                              .text
                          }</p>
